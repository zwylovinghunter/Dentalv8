...
from .metrics import bbox_iou, nwd_loss, piou_loss, probiou
...
class BboxLoss(nn.Module):
    ...
    def __init__(self, reg_max: int = 16, box_loss: str = "ciou"):
        """Initialize the BboxLoss module with regression loss and DFL settings."""
        super().__init__()
        self.box_loss = str(box_loss or "ciou").lower()
        if self.box_loss not in {"ciou", "piou", "hybrid_piou", "ciou_nwd_cls01"}:
            raise ValueError(f"Unsupported box_loss={box_loss}. Expected 'ciou', 'piou', 'hybrid_piou' or 'ciou_nwd_cls01'.")
    ...
    def forward(
        ...
        if self.box_loss == "piou":
            loss_iou_raw = piou_loss(pred_fg, target_fg, xywh=False)
        else:
            iou = bbox_iou(pred_fg, target_fg, xywh=False, CIoU=True)
            ciou_raw = 1.0 - iou
            if self.box_loss == "hybrid_piou":
                piou_raw = piou_loss(pred_fg, target_fg, xywh=False)
                target_cls = target_scores[fg_mask].argmax(-1)
                alpha_map = pred_fg.new_tensor([0.5, 0.5, 0.0])
                alpha = alpha_map[target_cls.clamp(max=alpha_map.numel() - 1)].unsqueeze(-1)
                loss_iou_raw = (1.0 - alpha) * ciou_raw + alpha * piou_raw
            elif self.box_loss == "ciou_nwd_cls01":
                nwd_raw = nwd_loss(pred_fg, target_fg, xywh=False)
                target_cls = target_scores[fg_mask].argmax(-1)
                lambda_map = pred_fg.new_tensor([0.2, 0.2, 0.0])
                lambda_cls = lambda_map[target_cls.clamp(max=lambda_map.numel() - 1)].unsqueeze(-1)
                loss_iou_raw = ciou_raw + lambda_cls * nwd_raw
            else:
                loss_iou_raw = ciou_raw
...
class v8DetectionLoss:
    ...
    def __init__(
        ...
        self.bbox_loss = BboxLoss(m.reg_max, getattr(h, "box_loss", "ciou")).to(device)
