...
def piou_loss(
    ...
    """Calculate PIoU v1 box regression loss.

    This follows the Powerful-IoU target-size-adaptive penalty. It returns a loss term, not an IoU similarity:
    loss = 1 - IoU + (1 - exp(-P^2)).
    """
    if xywh:
        (x1, y1, w1, h1), (x2, y2, w2, h2) = box1.chunk(4, -1), box2.chunk(4, -1)
        w1_half, h1_half, w2_half, h2_half = w1 / 2, h1 / 2, w2 / 2, h2 / 2
        b1_x1, b1_y1, b1_x2, b1_y2 = x1 - w1_half, y1 - h1_half, x1 + w1_half, y1 + h1_half
        b2_x1, b2_y1, b2_x2, b2_y2 = x2 - w2_half, y2 - h2_half, x2 + w2_half, y2 + h2_half
    else:
        b1_x1, b1_y1, b1_x2, b1_y2 = box1.chunk(4, -1)
        b2_x1, b2_y1, b2_x2, b2_y2 = box2.chunk(4, -1)
    ...
    dh1 = (b1_y1 - b2_y1).abs()
    dh2 = (b1_y2 - b2_y2).abs()
    p = (dw1 / w2 + dw2 / w2 + dh1 / h2 + dh2 / h2) / 4
    penalty = 1 - torch.exp(-(p.pow(2)))
    return (1 - iou + penalty).clamp(min=0)
...
def piou(
    ...
    """Official-compatible Powerful-IoU helper that returns a loss tensor.

    PIoU returns PIoU v1 loss. PIoU2 applies the official non-monotonic focusing factor to PIoU v1 loss.
    """
    loss = piou_loss(box1, box2, xywh=xywh, eps=eps)
    if PIoU2:
        if xywh:
            (x1, y1, w1, h1), (x2, y2, w2, h2) = box1.chunk(4, -1), box2.chunk(4, -1)
            w1_half, h1_half, w2_half, h2_half = w1 / 2, h1 / 2, w2 / 2, h2 / 2
            ...
            b1_x1, b1_y1, b1_x2, b1_y2 = box1.chunk(4, -1)
            b2_x1, b2_y1, b2_x2, b2_y2 = box2.chunk(4, -1)
        w2 = (b2_x2 - b2_x1).abs().clamp(min=eps)
        h2 = (b2_y2 - b2_y1).abs().clamp(min=eps)
        p = ((b1_x1 - b2_x1).abs() / w2 + (b1_x2 - b2_x2).abs() / w2 +
             (b1_y1 - b2_y1).abs() / h2 + (b1_y2 - b2_y2).abs() / h2) / 4
        x = torch.exp(-p) * Lambda
        return 3 * x * torch.exp(-(x.pow(2))) * loss
    if PIoU:
        return loss
    return loss
...
def nwd_loss(
    ...
    """Calculate Normalized Wasserstein Distance auxiliary loss for box regression.

    The default constant=12.8 follows common small-object detection settings. For imgsz=960 this may be tuned later,
    but this experiment keeps it fixed to isolate the class-aware NWD auxiliary term.
    """
    if xywh:
        x1, y1, w1, h1 = box1.chunk(4, -1)
        x2, y2, w2, h2 = box2.chunk(4, -1)
        c1x, c1y, c2x, c2y = x1, y1, x2, y2
        w1, h1, w2, h2 = w1.clamp(min=eps), h1.clamp(min=eps), w2.clamp(min=eps), h2.clamp(min=eps)
    else:
        b1_x1, b1_y1, b1_x2, b1_y2 = box1.chunk(4, -1)
        b2_x1, b2_y1, b2_x2, b2_y2 = box2.chunk(4, -1)
    ...
    distance = torch.sqrt((c1x - c2x).pow(2) + (c1y - c2y).pow(2) + ((w1 - w2).pow(2) + (h1 - h2).pow(2)) / 4 + eps)
    nwd = torch.exp(-distance / max(constant, eps))
    return (1.0 - nwd).clamp(min=0)
