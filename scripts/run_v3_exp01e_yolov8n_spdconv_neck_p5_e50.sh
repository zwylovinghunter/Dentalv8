...
cp "${PROJECT_FILES}/configs/yolov8n-dentex-v3-spdconv-neck-p5.yaml" "${REPO_ROOT}/configs/yolov8n-dentex-v3-spdconv-neck-p5.yaml"
cp "${PROJECT_FILES}/data/dentex_yolov8_3cls_v3.yaml" "${REPO_ROOT}/data/dentex_yolov8_3cls_v3.yaml"
cd "${REPO_ROOT}"
RUN_NAME="v3_exp01e_yolov8n_spdconv_neck_p5_e50_i960_b40"
SCREEN_NAME="v3_exp01e_spdconv_neck_p5"
...
screen -dmS "${SCREEN_NAME}" bash -lc "cd ${REPO_ROOT} && ${PYTHON_BIN} scripts/train_ablation.py --model configs/yolov8n-dentex-v3-spdconv-neck-p5.yaml --weights yolov8n.pt --data data/dentex_yolov8_3cls_v3.yaml --epochs 50 --imgsz 960 --batch 40 --device 0 --workers 16 --project ${REPO_ROOT} --name ${RUN_NAME} --seed 42 --patience 0 --optimizer auto --cos-lr False --amp True --cache ram --close-mosaic 10 --plots False > ${LOG_PATH} 2>&1"
