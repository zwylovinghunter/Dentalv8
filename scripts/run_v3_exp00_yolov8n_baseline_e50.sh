...
if [ ! -d "${REPO_ROOT}" ]; then
  ...
  echo "Restore/upload the YOLOv8 runtime source there, or run with REPO_ROOT=/path/to/yolov8."
  exit 1
fi
mkdir -p "${REPO_ROOT}/configs" "${REPO_ROOT}/data" "${REPO_ROOT}/logs"
cp "${PROJECT_FILES}/configs/yolov8n-dentex-v3-baseline.yaml" "${REPO_ROOT}/configs/yolov8n-dentex-v3-baseline.yaml"
cp "${PROJECT_FILES}/data/dentex_yolov8_3cls_v3.yaml" "${REPO_ROOT}/data/dentex_yolov8_3cls_v3.yaml"
cd "${REPO_ROOT}"
RUN_NAME="v3_exp00_yolov8n_baseline_e50_i960_b40"
SCREEN_NAME="v3_exp00_baseline"
...
screen -dmS "${SCREEN_NAME}" bash -lc "cd ${REPO_ROOT} && ${PYTHON_BIN} scripts/train_ablation.py --model configs/yolov8n-dentex-v3-baseline.yaml --weights yolov8n.pt --data data/dentex_yolov8_3cls_v3.yaml --epochs 50 --imgsz 960 --batch 40 --device 0 --workers 16 --project ${REPO_ROOT} --name ${RUN_NAME} --seed 42 --patience 0 --optimizer auto --cos-lr False --amp True --cache ram --close-mosaic 10 --plots False > ${LOG_PATH} 2>&1"
